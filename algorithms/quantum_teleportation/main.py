...
def algorithm(qb_count: int) -> qk.QuantumCircuit:
    ...
    circuit.cx(regQuantum[1], regQuantum[2])
    circuit.cx(regQuantum[0], regQuantum[1])
    circuit.h(regQuantum[0])
    circuit.measure(regQuantum[0], regClassic[0])
    circuit.measure(regQuantum[1], regClassic[1])
    circuit.cx(regQuantum[1], regQuantum[2])
    circuit.h(regQuantum[2])
    circuit.cz(regQuantum[0], regQuantum[2])
    circuit.h(regQuantum[2])
